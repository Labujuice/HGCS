...
- **檔案變更**：[Map.tsx](file:///home/kenny/Git_KennySpace/HGCS/web-ui/src/components/Map.tsx) 的雙擊事件分流與單擊 deselect 最佳化。

#### 2. MAVLink 2.0 協議參數與單位閉環 Review
- **經緯度座標修正**：修改了後端 [gateway.py](file:///home/kenny/Git_KennySpace/HGCS/gateway/gateway.py) 中的 `_handle_go_to` (DO_REPOSITION) 與 `_handle_orbit` (DO_ORBIT)，將原本的 `int(lat * 1e7)` 與 `int(lon * 1e7)` 徹底修正為符合 MAVLink 2.0 COMMAND_LONG 規範的單精度浮點數 **`float(lat)`** 與 **`float(lon)`**（單位為度 degree）。
- **指令格式優化**：
...
- **實作結果**：於 [surveyGenerator.ts](file:///home/kenny/Git_KennySpace/HGCS/web-ui/src/utils/surveyGenerator.ts) 中，獨立實作了 boustrophedon 掃描線求交幾何演算法（轉換至平面投影 $\rightarrow$ 逆旋轉 $-\theta$ $\rightarrow$ 劃平行線求交點並排序配對 $\rightarrow$ S-curve 連接 $\rightarrow$ 順旋轉 $\theta$ $\rightarrow$ 反投影回經緯度 `toFixed(6)` 輸出）。
