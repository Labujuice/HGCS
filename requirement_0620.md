...
* [ ] **【前端】** 實作類 QGC 界面配置，讓地圖幾乎佈滿整個畫面，提供沉浸式體驗。
* [ ] **【前端】** 在地圖上新增「快速定位載具 (Locate Drone)」按鈕，快速置中無人機。
* [ ] **【前端】** 右上角新增連線設定選單，可設定地端代理連線實體載具的通訊配置（下層軟體連線）。
* [ ] **【前端】** 下層連線選單支援 **UDP、TCP、Serial (串口)** 三種連接模式，包含連接地址、埠口、鮑率等設定。
* [ ] **【系統】** 支援管理與監控多台載具 (Multi-Vehicle Connection)，能接收多台遙測訊號並可切換當前主控載具。
